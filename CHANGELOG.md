...
- Replaced model-authored offsets, patch planning, labels, confidence, and explanations with one
  ...
  with the same `diff` library family used by Localix IDE.
...
- Matched Localix IDE's font boundary by shipping only Inter Latin and Latin Extended subsets for
  the English UI, with system fallbacks for other model-name scripts.
