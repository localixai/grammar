...
The repositories below were found through local repository search and GitHub MCP search, cloned,
and inspected at the recorded revisions.
...
| Localix IDE                                                                   | `6845a15d8446`    | Localix design tokens, bundled Inter typography, naming and product voice; `diff`-based deterministic hunk review; agent-runtime needs contrasted with Grammar's narrow call |
...
- `localix-dev/apps/api/src/features/agent-edits/hunk-patch.ts`,
  `localix-dev/packages/agent-runtime-pi/`, and the Localix theme sources. The IDE's agent loop is a
  different product need, while its deterministic diff-review pattern applies directly to Grammar.
...
| Localix IDE computes deterministic hunks locally with `diff` and exposes whole-change plus per-hunk decisions.                                                               | The model returns one complete corrected text. Grammar validates it and derives review spans locally with exactly pinned `diff@9.0.0`, leaving the model no offset, uniqueness, or patch-planning work.                                                                                                                                                                          | Corrected-text/diff unit tests, DOM apply tests, and browser E2E |
...
| Localix IDE needs an agent runtime, while Grammar makes one forced-tool request.                                                                                             | Grammar uses the official `@openrouter/sdk` directly. An agent framework would add unused loop, tool execution, and registry abstractions and previously required MV3-specific workarounds.                                                                                                                                                                                      | Grammar/model/OAuth tests and dist validator                     |
| Localix IDE uses a neutral OKLCH palette, monochrome focus ring, and Latin/Latin Extended Inter subsets for English UI.                                                      | Popup and overlay share the same visual vocabulary and do not depend on network fonts. The popup ships the same bounded Inter subsets, retains system fallbacks, and contrast-hardens small model metadata.                                                                                                                                                                      | Production build, screenshots, and axe E2E                       |
...
the report against the exact editor snapshot. Pi was investigated because Localix IDE legitimately
uses its agent runtime, but even its smaller AI layer brought an additional model abstraction and
browser-bundling accommodations that do not create product value here.
